...
export class UserController {
  ...
  // users/list API filter pagination
  @Post('/stateslist')
  public async statelist(@Body() request: Record<string, any>) {
    const { filters } = request;
    ...
    var data = {
      query: `query statelist() {
          states_aggregate(where:{${query}}) {
            aggregate {
              count
            }
          }
    ...
    let result = response?.data?.states;

    let mappedResponse = result;
    const count = response?.data?.states_aggregate?.aggregate?.count;
