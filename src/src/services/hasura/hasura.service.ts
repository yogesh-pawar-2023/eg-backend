...
export class HasuraService {
    ...
    constructor(private configService: ConfigService, private readonly httpService: HttpService, private qgService: QueryGeneratorService) { }
    ...
    public async getAll(
        ...
        if (!errors) {
            ...
            if (limit) {
                const totalCount = data?.[`${tableName}_aggregate`]?.aggregate?.count;
    ...
    public async qM(tableName: String, item: any, fields: any, onlyFields: any = []) {
        return this.getResponce(
            await lastValueFrom(
                this.httpService
                    .post(
